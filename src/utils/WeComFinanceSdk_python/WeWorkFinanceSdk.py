...
# 请注意更改路径
if os.name == "nt":
    sdk_dll = ctypes.CDLL("src/utils/WeComFinanceSdk_python/windows/WeWorkFinanceSdk.dll")
elif os.name == 'posix':
    sdk_dll = ctypes.CDLL("src/utils/WeComFinanceSdk_python/linux-x86/libWeWorkFinanceSdk_C.so")
else:
...
if __name__ == "__main__":
    ...
    corp_key = os.environ.get('secret', '')
    # 有私钥才能看明文
    prikey_pem_path = os.environ.get('prikey_path', '')
    start_seq = 0
    record_limit = 10
    has_prikey = False
    try:
        ...
        # 导入私钥
        if os.path.exists(prikey_pem_path):
            with open(prikey_pem_path) as pk_file:
